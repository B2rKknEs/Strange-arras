...
exports.addShield = (damageFactor = 1, sizeFactor = 1, opacity = 0.3, auraColor) => {
    let isHeal = damageFactor < 0;
    let shieldType = isHeal ? "healAura" : "shield";
    let symbolType = isHeal ? "healerSymbol" : "shieldSymbol";
    ...
    return {
        ...
        GUNS: [
            {
                POSITION: [0, 20, 1, 0, 0, 0, 0,],
                PROPERTIES: {
                    SHOOT_SETTINGS: exports.combineStats([g.shield, { size: sizeFactor, damage: 0 }]),
        ...
        TURRETS: [
            {
                POSITION: [20 - 7.5 * isHeal, 0, 0, 0, 360, 1],
                TYPE: [symbolType, {INDEPENDENT: true}],
            },
        ]
    };
}
...
exports.makeRare = (type, level) => {
    ...
    return {
        PARENT: "food",
        LABEL: ["Shiny", "Legendary", "Shadow", "Rainbow", "Trans", "Albino", "Epilepsy", "Black"][level] + " " + type.LABEL,
        VALUE: [100, 500, 2000, 4000, 5000, 10000, 30000, 250000][level] * type.VALUE,
        SHAPE: type.SHAPE,
        SIZE: type.SIZE,
        COLOR: ["lightGreen", "teal", "darkGrey", "rainbow", "trans", "white", "police", "#000000"][level],
        ALPHA: level == 2 ? 0.25 : 1,
        BODY: {
            DAMAGE: [1, 1, 2, 2.5, 2.5, 3, 5, 8][level] * type.BODY.DAMAGE,
            DENSITY: [1, 1, 2, 2.5, 2.5, 3, 4, 5][level] * type.BODY.DENSITY,
            HEALTH: [2, 4, 4, 6, 8, 12, 24, 200][level] * type.BODY.HEALTH,
            PENETRATION: [1.5, 1.5, 2, 2.5, 2.5, 3, 3, 3.5][level] * type.BODY.PENETRATION,
...
exports.makeLaby = (type, level, baseScale = 1) => {
    ...
    return {
        ...
        DRAW_HEALTH: type.DRAW_HEALTH || level > 1,
