...
cNestTick = tile => {
    if (++tile.data.enemySpawnCooldown > Config.ENEMY_SPAWN_COOLDOWN_CNEST) {
        tile.data.enemySpawnCooldown = 0;
        if (tile.data.enemyCount < Config.ENEMY_CAP_CNEST && Math.random() < Config.ENEMY_SPAWN_CHANCE_CNEST) {
            spawnNatural(tile, Config.ENEMY_TYPES_CNEST, 'enemy');
        }
    }
},
...
cNestColor = {BASE: "pink", BRIGHTNESS_SHIFT: 5, SATURATION_SHIFT: 0.8},
crasherNest = new Tile({
    color: cNestColor,
    data: {
        allowMazeWallSpawn: true,
        foodSpawnCooldown: 0, foodCount: 0,
...
module.exports = { normal, nest, crasherNest, dangerNest, wall, gunWall, nestNoBoss, crasherNestNoBoss, dangerNestNoBoss };